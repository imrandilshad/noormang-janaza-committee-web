...
async function fetchDashboardStats(): Promise<DashboardStats> {
  ...
  const collectionRows = (collections.data ?? []) as {
    amount_due: number;
    amount_paid: number;
    status: string;
  }[];
...
export function DashboardPage() {
  ...
  const collectionRate =
    stats && stats.totalExpenses > 0
      ? Math.min(
          100,
          Math.round((stats.totalCollections / stats.totalExpenses) * 100),
        )
      : 0;
                  ...
                  <div className="divide-y">
                    {recentCases.map((c) => (
                      ...
                      >
                        ...
                        <Badge
                          variant={
                            c.status === "open" ? "destructive" : "outline"
                          }
                          className="shrink-0 min-w-[64px] flex justify-center items-center"
                        >
                          {c.status}
                        </Badge>
                      </div>
