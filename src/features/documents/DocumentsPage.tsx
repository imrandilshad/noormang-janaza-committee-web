...
const documentSchema = z.object({
  ...
  file_url: z.string().min(1, "Please upload a file or paste a URL"),
...
export function DocumentsPage() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.error(
        "Upload failed",
        'Ensure a public storage bucket named "documents" exists in Supabase.',
      );
    } finally {
      setUploading(false);
  ...
  const upsert = useMutation({
    mutationFn: async (values: DocumentForm) => {
      const caseId =
        values.funeral_case_id && values.funeral_case_id !== "_none"
          ? values.funeral_case_id
          : null;
      const payload = {
        title: values.title,
    ...
    onSuccess: () => {
      ...
      setEditing(null);
      toast.success(
        editing ? "Document updated" : "Document added",
        "Saved successfully.",
      );
    },
  ...
  const openEdit = (d: Document) => {
    ...
    reset({
      ...
      file_url: d.file_url,
      funeral_case_id: d.funeral_case_id ?? "_none",
    });
    setOpen(true);
  };
  const openAdd = () => {
    setEditing(null);
    reset({ document_type: "other", funeral_case_id: "_none" });
    setOpen(true);
  };
  ...
  const filtered = documents.filter(
    (d) =>
      ...
      (
        (d as Document & { funeral_cases: { case_number: string } | null })
                  ...
                  filtered.map((d) => (
                    <TableRow key={d.id}>
                      ...
                      <TableCell>
                        <Badge
                          variant={
                            typeVariant[d.document_type] as
                              | "default"
                              | "secondary"
                              | "destructive"
                              | "outline"
                          ...
                          <AlertDialog>
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                  Delete "<strong>{d.title}</strong>"? This
                                  cannot be undone.
                                </AlertDialogDescription>
                              </AlertDialogHeader>
                              <AlertDialogFooter>
        ...
        <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto w-[calc(100vw-2rem)] sm:w-full">
          ...
          >
            <div className="space-y-2">
              ...
              <Input
                {...register("title")}
                placeholder="e.g. Death Certificate – Ahmad Khan"
              />
              {errors.title && (
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="meeting_minutes">
                    Meeting Minutes
                  </SelectItem>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Or paste a URL (Google Drive, Dropbox, etc.)
              </p>
              <Input
                {...register("file_url")}
                type="text"
            ...
            <div className="space-y-2">
              ...
              <Combobox
                ...
                value={watch("funeral_case_id") ?? "_none"}
                onValueChange={(v) => setValue("funeral_case_id", v)}
                placeholder="Link to a case (optional)"
