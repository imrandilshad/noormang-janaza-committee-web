...
export function CollectionsPage() {
  ...
  const { data: funeralCases = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (data ?? []) as { id: string; case_number: string; deceased_name: string; expenses: { amount: number }[] }[]
    },
  })
  ...
  const generateMutation = useMutation({
    mutationFn: async (caseId: string) => {
      ...
      if (activeMembers.length === 0) throw new Error('No active members found')

      const perMemberAmount = totalExpenses > 0 ? Number((totalExpenses / activeMembers.length).toFixed(2)) : 0
      ...
      if (newRecords.length === 0) throw new Error('Collection records already exist for all active members for this case')
  ...
  const filtered = collections.filter(
    (c) =>
      ...
      c.funeral_cases?.deceased_name?.toLowerCase().includes(search.toLowerCase()),
  )

  const selectedCaseData = funeralCases.find((c) => c.id === selectedCase)
  const selectedCaseTotal = selectedCaseData?.expenses?.reduce((s, e) => s + e.amount, 0) ?? 0
    ...
    <div className="space-y-4 pb-4">
      ...
      <div className="sticky top-0 z-20 bg-background pt-3 pb-2">
        <div className="flex items-start justify-between gap-2">
          <h1 className="text-xl sm:text-2xl font-bold leading-tight">{t('collection.collectionList')}</h1>
          <Button size="sm" onClick={() => setGenOpen(true)} className="shrink-0">
            <Plus className="mr-1.5 h-4 w-4" />
            <span className="hidden sm:inline">{t('collection.generateCollections')}</span>
            <span className="sm:hidden">{t('collection.generate')}</span>
          </Button>
        </div>
        <p className="text-xs text-muted-foreground mt-0.5 hidden sm:block">{t('collection.subtitle')}</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-3">
        <Card>
          <CardContent className="px-3 pt-3 pb-3 sm:pt-4 sm:pb-4">
            <p className="text-xs text-muted-foreground truncate">{t('collection.amountDue')}</p>
            <p className="text-sm sm:text-xl font-bold mt-0.5 truncate">{formatCurrency(totals.due)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="px-3 pt-3 pb-3 sm:pt-4 sm:pb-4">
            <p className="text-xs text-muted-foreground truncate">{t('collection.amountPaid')}</p>
            <p className="text-sm sm:text-xl font-bold text-green-600 mt-0.5 truncate">{formatCurrency(totals.paid)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="px-3 pt-3 pb-3 sm:pt-4 sm:pb-4">
            <p className="text-xs text-muted-foreground truncate">{t('collection.outstanding')}</p>
            <p className="text-sm sm:text-xl font-bold text-destructive mt-0.5 truncate">{formatCurrency(totals.due - totals.paid)}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Dialog open={genOpen} onOpenChange={setGenOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>{t('collection.generateDesc')}</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            ...
            {selectedCase && (
              <div className="rounded-md border bg-muted/40 p-3 space-y-1 text-sm">
                <p>{t('collection.totalExpenseForCase')}: <span className="font-semibold">{formatCurrency(selectedCaseTotal)}</span></p>
                <p className="text-xs text-muted-foreground">{t('collection.generateNote')}</p>
              </div>
            )}
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setGenOpen(false)}>{t('common.cancel')}</Button>
            <Button
