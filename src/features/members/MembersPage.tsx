...
export function MembersPage() {
  ...
  const { data: members = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .order('member_number')
      return (data ?? []) as (Member & { families: { family_name: string } | null })[]
    },
  })
  ...
  const { register, handleSubmit, reset, setValue, watch, formState: { errors, isSubmitting } } = useForm<MemberForm>({
    ...
    mutationFn: async (values: MemberForm) => {
      ...
      if (editing) {
        ...
        if (last && last.length > 0) {
          const n = parseInt((last[0] as { member_number: string }).member_number.replace(/\D/g, ''), 10)
  ...
  const openAdd = () => {
    setEditing(null)
    reset({ status: 'active', joined_date: new Date().toISOString().split('T')[0] })
    setOpen(true)
  }
    ...
    <div className="space-y-4">
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                  Delete <strong>{member.full_name}</strong> ({member.member_number})? This cannot be undone.
                                </AlertDialogDescription>
                              </AlertDialogHeader>
                              <AlertDialogFooter>
                                <AlertDialogCancel>{t('common.cancel')}</AlertDialogCancel>
                                <AlertDialogAction onClick={() => deleteMutation.mutate(member.id)}>{t('common.delete')}</AlertDialogAction>
                              </AlertDialogFooter>
                            </AlertDialogContent>
                          </AlertDialog>
                        </div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? t('member.editMember') : t('member.addMember')}</DialogTitle>
          </DialogHeader>
            ...
            <div className="grid grid-cols-2 gap-4">
              {editing && (
                <div className="space-y-2">
                  ...
                  <div className="font-mono text-sm px-3 py-2 border rounded-md bg-muted text-muted-foreground">{editing.member_number}</div>
                </div>
              )}
              <div className="space-y-2">
                <Label>{t('member.status')}</Label>
                <Select value={watch('status')} onValueChange={(v) => setValue('status', v as 'active' | 'inactive' | 'deceased')}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="active">{t('member.active')}</SelectItem>
                    <SelectItem value="inactive">{t('member.inactive')}</SelectItem>
                    <SelectItem value="deceased">{t('member.deceased')}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="space-y-2">
                <Label>{t('member.joinedDate')}</Label>
                <Input {...register('joined_date')} type="date" />
              </div>
              <div className="space-y-2">
                <Label>{t('member.family')}</Label>
                <Select value={watch('family_id') ?? ''} onValueChange={(v) => setValue('family_id', v)}>
